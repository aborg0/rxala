...
//  private[rxala] def wrappedNoBackPressure[BB <: B =:= NoBackPressure]: F#WrappedNoBackPressureObservableType[R, E, T]
  ...
  private[rxala] def extract(s: Subscriber[_ <: R, _ >: E, _ >: T, WithBackPressure, F]): F#WrappedSubscriberType[R, E, T] = s.asInstanceOf[BackPressureSubscriberWrapper[R, E, T, F]].obs
